...
namespace Day22
{
    class Program
    {
        ...
        static void SolvePart2()
        {
            ...
            int playerHP = 50;
            int mana = 500;
            int lowestMana = Fight2(hp, damage, playerHP, mana, 0, 0, 0, 0, true, int.MaxValue);
            Console.WriteLine("Lowest Mana = " + lowestMana);
        }
        ...
        static int Fight2(int bossHP, int BossDamage, int playerHP, int playerMana, int shield, int poison, int recharge, int manaCost, bool playerTurn, int lowestCost)
        {
            if (bossHP <= 0) return manaCost;
            if (playerHP <= 0) return -1;
            if (manaCost > lowestCost) return -1;
            if (!playerTurn)
            {
                ...
                poison--;
                recharge--;
                return Fight2(bossHP, BossDamage, playerHP, playerMana, shield, poison, recharge, manaCost, true, lowestCost);
            }
            else
            {
                playerHP--;
                ...
                foreach (var s in spells.Where(x => x.cost <= playerMana))
                {
                    ...
                    if (shield > 2 && s.shield > 0) continue;
                    if (recharge > 2 && s.recharge > 0) continue;
                    int tmp = Fight2(bossHP - s.damage, BossDamage, playerHP + s.heal, playerMana - s.cost, Math.Max(shield, s.shield), Math.Max(poison, s.poison), Math.Max(recharge, s.recharge), manaCost + s.cost, false, lowestCost);
                    if (tmp != -1)
                    {
                        lowestCost = Math.Min(lowestCost, tmp);
                    }
